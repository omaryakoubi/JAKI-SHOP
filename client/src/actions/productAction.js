...
export const filtreProducts=(searchkey, category)=>dispatch=>{
    ...
    axios.get('/api/products/allproducts').then(res=>{
        ...
        if(searchkey)
        {
            filtredproducts = res.data.filter(product => {return product.name.toLowerCase().includes(searchkey)})

        }

        if (category!=='all')
        {
            filtredproducts = res.data.filter(product => {return product.category.toLowerCase().includes(category)})
